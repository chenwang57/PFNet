...
def create_alias_table(area_ratio):
    ...
    :param area_ratio: np.sum(area_ratio)=1 ndarray
    :return: accept,alias
    """
...
class LINE:
    def __init__(self, matrix, embedding_size=8, negative_ratio=5, order='second', ):
        """

        :param matrix: numpy ndarray (need to convert to nx_graph)
        :param embedding_size:
        :param negative_ratio:
        :param order:
        """
        super().__init__()
        assert order in {'first', 'second', 'all'}, "order parameter must be first, second or all!"
        self.matrix = matrix
        self.graph = self.__convert_to_graph()
        ...
        self.edge_size = self.graph.number_of_edges()
        self.samples_per_epoch = self.edge_size * (1 + negative_ratio)

        self._gen_sampling_table()
        self.reset_model()

    def __convert_to_graph(self):
        ...
        for row in range(num_nodes):
            for col in range(num_nodes):
                if col != row:
                    graph.add_weighted_edges_from([(row, col, self.matrix[row, col])])
    ...
    def get_embeddings(self, is_convert=True):
        self._embeddings = {}
        if self.order == 'first':
        ...
        if is_convert:
            return self.__convert_to_ndarray()
        else:
            return self._embeddings

    def train(self, batch_size=1024, epochs=100, initial_epoch=0, verbose=1, times=1):
    ...
    def __convert_to_ndarray(self):
        emb_list = []
        for i in range(self.node_size):
            emb_list.append(self._embeddings[i])
        return np.array(emb_list, dtype=np.float32)


if __name__ == '__main__':
    distance_matrix = np.load('../original_dataset/ManchesterHW_drive_distance.npy')
    line = LINE(distance_matrix, embedding_size=128, order='second')
    hist = line.train()
    graph = line.get_graph()
    result = line.get_embeddings()
    np.save('../input/spatial_embedding_ManchesterHW.npy', result)
